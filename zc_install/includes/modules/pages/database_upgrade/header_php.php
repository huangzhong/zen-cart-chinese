...
*/
    ...
    if ($failed_entries !=0 ) {
      $zc_install->setError('<span class="errors">提示: 未执行的升级指令: '.$failed_entries.'<br />见页面底部的详细说明。<br />(详情也可查看 "upgrade_exceptions" 数据表.)</span><br />提示: 通常可以忽略这些未执行的指令，<br />也许您的网站已经执行过这些指令。<br />如果所有建议的升级步骤已经完成，<br />可以继续配置您的网站。','85', false);
    }
    if (ZC_UPG_DEBUG2==true) echo '<span class="errors">提示: 忽略的升级指令: '.$failed_entries.'<br />见页面底部的详细说明。<br />(详情也可查看 "upgrade_exceptions" 数据表.)</span>';

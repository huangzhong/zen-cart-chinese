...
UPDATE configuration SET configuration_description = '指向 configure.php 文件中 DIR_FS_SQL_CACHE 设定的目录。' WHERE configuration_key = 'SESSION_WRITE_DIRECTORY';

UPDATE configuration set configuration_title = '记录页面解析时间', configuration_description = '保存(到文件)解析页面的时间' WHERE configuration_key = 'STORE_PAGE_PARSE_TIME';
UPDATE configuration set configuration_title = '日志路径', configuration_description = '页面解析时间的日志目录和文件名' WHERE configuration_key = 'STORE_PAGE_PARSE_TIME_LOG';
UPDATE configuration set configuration_title = '日志时间格式', configuration_description = '时间格式' WHERE configuration_key = 'STORE_PARSE_DATE_TIME_FORMAT';
UPDATE configuration set configuration_title = '显示页面解析时间', configuration_description = '在每个页面底部显示页面解析时间<br />(说明: 显示解析时间并不要求保存。)' WHERE configuration_key = 'DISPLAY_PAGE_PARSE_TIME';
UPDATE configuration set configuration_title = '保存数据库查询', configuration_description = '保存数据库查询到系统的 /logs/ 目录。注意，这有可能严重影响系统性能并用光硬盘空间。' WHERE configuration_key = 'STORE_DB_TRANSACTIONS';

UPDATE configuration set configuration_title = '使用 HTML 邮件?', configuration_description = '是否使用 HTML 格式发送电子邮件' WHERE configuration_key = 'EMAIL_USE_HTML';
UPDATE configuration set configuration_title = '管理员邮件格式', configuration_description = '请选择管理员电子邮件格式' WHERE configuration_key = 'ADMIN_EXTRA_EMAIL_FORMAT';
...
UPDATE configuration set configuration_description = '输入以秒为单位的时间。<br />PCI 兼容要求最大不超过 900 。<br /> 默认=900<br />例如: 900= 15 分钟<br /><br />备注: 时间太短会导致添加/编辑商品时超时', use_function = '', set_function = '' where configuration_key = 'SESSION_TIMEOUT_ADMIN';
